...
export default async function AdminPage({
  ...
  if (!isAuthed) {
    if (searchParams.auth === process.env.ADMIN_PASSWORD) {
      // Will be set by middleware-style check, redirect to set cookie
      redirect(`/api/admin/auth?password=${searchParams.auth}`)
    }
    return (
              ...
              {pending.map((article) => (
                <div key={article.id} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '16px' }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: '11px', color: 'var(--teal-600)', fontWeight: 500, marginBottom: '6px' }}>
                        {article.category} · {article.source_name}
                        {article.valence !== null && (
                          <span style={{ marginLeft: '12px', color: 'var(--text-tertiary)', fontWeight: 400 }}>
                ...
                <div key={article.id} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '11px', color: 'var(--text-tertiary)', marginBottom: '4px' }}>
                      {article.category} · {article.featured ? 'Featured' : 'Standard'}
                    </div>
