...
export default async function AdminPage({
  ...
  const deskLabel = successDesk === 'apple' ? 'One Recursive Loop' : successDesk === 'market' ? 'The Old Market' : 'Techlomerate'
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'var(--bg)', padding: '40px' }}>
      <div style={{ maxWidth: '860px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '32px' }}>
          <div>
            ...
            <div style={{ fontSize: '12px', color: 'var(--text-tertiary)' }}>
              {pending?.length || 0} main · {applePending?.length || 0} apple · {marketPending?.length || 0} market pending
            </div>
          </div>
        ...
        {(!pending || pending.length === 0) && (!applePending || applePending.length === 0) && (!marketPending || marketPending.length === 0) && (
